...
function ideal_eq2(y_target) {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        if (y_guess < y_target) { low = mid_xa; } else { high = mid_xa; }
    }
    return (low + high) / 2;
}
...
function find_q_ideal_eq_intersect(q, zf) {
    ...
    let low = 0.0, high = 1.0, mid_x;
    for (let i = 0; i < 30; i++) {
        mid_x = (low + high) / 2;
        if (f(mid_x) < 0) { low = mid_x; } else { high = mid_x; }
    }
...
function find_x_from_data(dataArray, y_target) {
    ...
    if (y_target >= dataArray[dataArray.length - 1].y) return dataArray[dataArray.length - 1].x;
...
export function calculateMcCabeThiele(variables) {
    const { xd, xb, zf, q, r_factor, nm } = variables;
    const plotData = {
        ideal_eq_curve: [],
        line_45: [{x: 0, y: 0}, {x: 1, y: 1}],
        q_line: [],
        esol_line: [],
        ssol_line: [],
        steps: [],
    ...
    if (Math.abs(q - 1.0) < 1e-6) {
        ...
        const q_slope = q / (q - 1);
        const q_intercept = -zf / (q - 1);
        q_esol_intersect_x = (esol_intercept - q_intercept) / (q_slope - esol_slope);
        q_esol_intersect_y = esol_slope * q_esol_intersect_x + esol_intercept;
    }
    ...
    plotData.q_line = [{x: zf, y: zf}, {x: q_ideal_eq_x, y: q_ideal_eq_y}];
    plotData.esol_line = [{x: xd, y: xd}, {x: q_esol_intersect_x, y: q_esol_intersect_y}];
    plotData.ssol_line = [{x: xb, y: xb}, {x: q_esol_intersect_x, y: q_esol_intersect_y}];
    ...
    while (current_x > xb && stage_count < MAX_STAGES) {
        ...
        plotData.steps.push([ {x: current_x, y: step_start_y}, {x: next_x, y: step_start_y} ]); // 수평선
        plotData.steps.push([ {x: next_x, y: step_start_y}, {x: next_x, y: next_y} ]); // 수직선
